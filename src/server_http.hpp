...
/* WEB SERVER IS BASED ON https://github.com/eidheim/Simple-Web-Server */
...
namespace SimpleWeb {
	...
	public:
		...
		private:
			...
			Response(socket_type &socket, boost::asio::yield_context& yield): std::ostream(&streambuf), yield(yield), socket(socket) {}
		...
		public:
			size_t size() { return streambuf.size(); }
			std::string string() { std::stringstream ss; ss << rdbuf(); return ss.str(); }
		private:
			boost::asio::streambuf &streambuf;
		...
		class Config {
			friend class ServerBase<socket_type>;
		private:
			Config(const std::string& address, unsigned short port, size_t num_threads): address(address), port(port), num_threads(num_threads), reuse_address(true) {}
			std::string address;
			unsigned short port;
			size_t num_threads;
		public:
	...
	public:
		void start() {
			...
			if(config.address.size()>0) endpoint=boost::asio::ip::tcp::endpoint(boost::asio::ip::address::from_string(config.address), config.port);
			else endpoint=boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), config.port);
	...
	protected:
		...
		ServerBase(const std::string& address, unsigned short port, size_t num_threads, size_t timeout_request, size_t timeout_send_or_receive) : 
				config(address, port, num_threads), acceptor(io_service),
		...
		void read_request_and_content(std::shared_ptr<socket_type> socket) {
			...
			if(timeout_request>0) timer=set_timeout_on_socket(socket, timeout_request);
				...
				if((path_end=line.find(' ', method_end+1))!=std::string::npos) {
					...
					while((param_end=line.find(':'))!=std::string::npos) {
						...
						if((value_start)<line.size()) {
							...
							if(value_start<line.size()) request->header.insert(std::make_pair(line.substr(0, param_end), line.substr(value_start, line.size()-value_start-1)));
						}
		...
		void find_resource(std::shared_ptr<socket_type> socket, std::shared_ptr<Request> request) {
			...
			if(it_method!=default_resource.end()) write_response(socket, request, it_method->second);
		}
		...
		void write_response(std::shared_ptr<socket_type> socket, std::shared_ptr<Request> request, 
			...
			std::shared_ptr<boost::asio::deadline_timer> timer;
			if(timeout_content>0) timer=set_timeout_on_socket(socket, request, timeout_content);
			...
			boost::asio::spawn(request->strand, [this, &resource_function, socket, request, timer](boost::asio::yield_context yield) {
				...
				if(http_version>1.05) read_request_and_content(socket);
			});
		}
	};
	...
	public:
		Server(const std::string address, unsigned short port, size_t num_threads=1, size_t timeout_request=5, size_t timeout_content=300) : 
